...
- [ ] Build the serving image after Docker Desktop / daemon is running:
  ```bash
  docker compose -f docker/docker-compose.yaml build
  ```
  Current local attempt: blocked by `Cannot connect to the Docker daemon`.

- [ ] Start the serving stack and inspect health:
  ```bash
...
- [ ] Confirm no generated secrets or local artefacts are staged:
  ```bash
  git status --short
  ```
